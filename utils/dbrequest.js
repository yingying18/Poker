...
/* User DB request function */
  ...
  changePassword: function (con, data, callback) {
    console.log(colors.yellow("data paremeter passed to db call changePassword : " + JSON.stringify(data) + '\n'));
    let result = con.query("UPDATE user SET password = '" + data.password + "' WHERE id = '" + data.userid + "'",
      function (err, result, fields) {
        if (err) {
          console.log(colors.magenta("db error : changePassword ->" + JSON.stringify(result) + '\n'));
          callback(err);
          throw err;
        }
        console.log(colors.yellow("returning response from db changePassword : " + JSON.stringify(result) + '\n'));
        callback(result);
        //con.release();
      }
    );
  ...
  deleteUser: function (con, data, callback) {
    console.log(colors.yellow("data paremeter passed to db call deleteUser : " + JSON.stringify(data) + '\n'));
    let result = con.query("DELETE FROM user where id = " + data.id,
      function (err, result, fields) {
        if (err) {
          console.log(colors.magenta("db error : updateUser ->" + JSON.stringify(result) + '\n'));
          callback(err);
